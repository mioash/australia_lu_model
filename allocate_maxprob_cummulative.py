...
def max_prob_allocator_v1 (datafile,column_future,demand_file,demand):
    
    slap = datafile
    ##calculate demand based on future data // when it is available
    
    if demand_file:
        LU0_targ = int(demand.crop)
        LU1_targ = int(demand.forest)
        LU2_targ = int(demand.grass)
        LU3_targ = int(demand.urban)
    else:
...
#    n_df = n_df[n_df[:,column_future+1] != -9999,:]
    ...
    if len(pos_d) == 0 or len(neg_d) == 0:
        ...
        if demand_file:
            ...
            outarray[:, 2] = df_end[:, column_future-1] #prev
            outarray[:, 3] = df_end[:,column_future+1] #predicted
            outarray[:, 4] = df_end[:,2] #geoid
            #outarray[:, 3] = datafile[:,]
            return outarray#, len(pos_d),len(neg_d)
