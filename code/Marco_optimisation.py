...
# Latitude, Longitude, Probability of being Crop (Pcrop), Pforest, Pgrassland, Purban, Pother, LU_2005, LU_2010
datafile = np.load('N:/Planet-A/Current-Users/Brett-Bryan/Marco/sla_601021010.npy')

# Get rid of nodata rows and rows where LU2015 = water
datafile = datafile[(datafile[:, 6] != -9999) & 
                    (datafile[:, 7] != 4) & 
                    (datafile[:, 7] != 5) & 
                    (datafile[:, 8] != 4) & 
                    (datafile[:, 8] != 5)
...
LU0_targ = np.sum(datafile[:, 8] == 0) # 1334
LU1_targ = np.sum(datafile[:, 8] == 1) # 40083
LU2_targ = np.sum(datafile[:, 8] == 2) # 5116
LU3_targ = np.sum(datafile[:, 8] == 3) # 6661
...
print('Accuracy = ', np.sum(x == datafile[:, 8]) / numcells)
...
np.save('N:/Planet-A/Current-Users/Brett-Bryan/Marco/gurobi_output.npy', outarray)
