# -*- coding: utf-8 -*-
"""
Created on Mon Aug 16 14:44:16 2021
...
if uuser=='mioash':
    ...
    csuit_crop = ee.Image('users/'+str(uuser)+'/'+str(year)+'_SUIT_AUS_v3_25k_150trees_cropland')
    psuit_crop = ee.Image('users/'+str(uuser)+'/'+str(year-5)+'_SUIT_AUS_v3_25k_150trees_cropland')
    imc = ee.ImageCollection([csuit_crop,psuit_crop])
    suit_crop = imc.reduce(ee.Reducer.max())
    
    #users/mioash/Calderon_etal_Australian_land-cover/2010_SUIT_AUS_v25k_forest
    csuit_forest = ee.Image('users/'+str(uuser)+'/'+str(year)+'_SUIT_AUS_v3_25k_150trees_forest')
    psuit_forest = ee.Image('users/'+str(uuser)+'/'+str(year-5)+'_SUIT_AUS_v3_25k_150trees_forest')
    imf = ee.ImageCollection([csuit_forest,psuit_forest])
    suit_forest = imf.reduce(ee.Reducer.max())
    
    csuit_grass = ee.Image('users/'+str(uuser)+'/'+str(year)+'_SUIT_AUS_v3_25k_150trees_grassland')
    psuit_grass= ee.Image('users/'+str(uuser)+'/'+str(year-5)+'_SUIT_AUS_v3_25k_150trees_grassland')
    img = ee.ImageCollection([csuit_grass,psuit_grass])
    suit_grass = img.reduce(ee.Reducer.max())
    
    csuit_urb = ee.Image('users/'+str(uuser)+'/'+str(year)+'_SUIT_AUS_v3_25k_150trees_urban')
    psuit_urb = ee.Image('users/'+str(uuser)+'/'+str(year-5)+'_SUIT_AUS_v3_25k_150trees_urban')
    imu = ee.ImageCollection([csuit_urb,psuit_urb])
    suit_urb = imu.reduce(ee.Reducer.max())
...
def main_process (stt,slas,loc_past,loc_future,iq,ind_grid):
    
    if ind_grid:
        sla_id = int(slas[slas == ind_grid])
        #sla_id = int(slos[34])#int(slos[slos == 601061035])
    else:
        sla_id = int(slas[iq])
    ...
    dfmo1, acco1, conf1 = max_prob_allocator_v1(slap,loc_future)
...
#    import matplotlib.pyplot as plt
    ...
    npy_to_tif(finn_a,0,1,3,'E:/Marco/australia_lu_model/data/allocation/'+str(stt)+'/'+str(year)+'m/pred_'+str(year)+'_'+str(sla_id)+'_v3.tiff')  #predicted is on number 3
    ...
    npy_to_tif(finn_a1,0,1,3,'E:/Marco/australia_lu_model/data/allocation/'+str(stt)+'/'+str(year)+'am/pred_'+str(year)+'_'+str(sla_id)+'_v3_test_stable.tiff')  #predicted is on number 3
    
    
    # finn_a2 = np.concatenate((dfmo3,nan_df[:,[0,1,loc_future,loc_future+1,2]]),axis=0)
    ...
    gc.collect()
    conf.to_csv('E:/Marco/australia_lu_model/data/allocation/'+str(stt)+'/'+str(year)+'m/'+str(year)+'_cf_'+str(sla_id)+'.csv')    
    conf1.to_csv('E:/Marco/australia_lu_model/data/allocation/'+str(stt)+'/'+str(year)+'am/'+str(year)+'_cfa_'+str(sla_id)+'.csv')    
...
for ist in range(0,len(STTs)):
    ...
    if STT == 'New South Wales':        
        ...
        stt = 'sa'
        rem_sla = [406021141,406011135,406021138,407031161,406011134,406011131,406011130,406011132]    ##faltan todos (aumentar una grande de ~73 grids)
        for i in range(0,len(rem_sla)):            
            slas1 =slas1[slas1 != rem_sla[i]]
        ext_sla = 1
    ...
    Parallel(n_jobs=30,prefer='threads')(delayed(main_process)(stt,slas1,loc_past,loc_future,iq,False) for iq in range(0,len(slas1)))#range(0,len(slas1)))
    ...
    for qqj in range(0,len(slas1)):
        if os.path.isfile('E:/Marco/australia_lu_model/data/allocation/'+str(stt)+'/'+str(year)+'/pred_'+str(year)+'_'+str(slas1[qqj])+'_v3.tiff'):
            a=1
            #print('exists')#main_process(stt,slas1,loc_past,loc_future,0,slas1[qqj])
        else:
            #print(qqj,'to_dwn')
            main_process(stt,slas1,loc_past,loc_future,0,slas1[qqj])
    #rem_sla = [113011257]            
    if ext_sla == 1:
        for j in range(0,len(rem_sla)):#len(rem_sla)
            if os.path.isfile('E:/Marco/australia_lu_model/data/allocation/'+str(stt)+'/'+str(year)+'m/pred_'+str(year)+'_'+str(rem_sla[j])+'_v3.tiff'):
                a=1
            else:
                main_process(stt,slos,loc_past,loc_future,0,rem_sla[j])
